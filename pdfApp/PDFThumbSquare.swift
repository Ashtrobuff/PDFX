...
struct PDFThumbSquare: View {
    ...
    var body: some View {
        ...
        VStack{
            model.pdfThumbnail.frame(width:100,height:50).clipped()
            ...
            HStack{
                ...
                Menu {
                    Button("Order Now", action: {})
                           ...
                           Image(systemName: "ellipsis").tint(.secondary).rotationEffect(.degrees(90)).fontWeight(.ultraLight).frame(width:20,height:20)
                       }
            }
            
        }.frame(width:100,height:150).padding(.bottom,10)
        }.padding(2).overlay(
