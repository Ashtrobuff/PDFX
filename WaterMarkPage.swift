...
struct WaterMarkPage: View {
    ...
    var body: some View {
        Form{
            ZStack{
                HStack{
                    Spacer()
                    Text(text).rotationEffect(.degrees(rotation)).id(resVersion)
                    Spacer()
    ...
    func applyAttributes()
    {
        ...
        self.text.setAttributes([.foregroundColor:UIColor(color),.font:UIFont.systemFont(ofSize: 36, weight: weight)],range: NSRange(location:0,length:text.length))
